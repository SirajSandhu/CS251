...
__global__ void calculate(char *mem, int num, int cols) {
      int thread_id = threadIdx.x + cols * threadIdx.y;   //flattened thread id value
      int i = blockDim.x * blockDim.y * blockIdx.x + thread_id;
...
int main(int argc, char **argv) {
    ...
    calculate<<<blocks, threadsPerBlock >>>(gpu_mem, num, cols);
